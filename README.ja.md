...
汎用的な Linux desktop 対応は作業中です。現時点では Loupedeck Live S を対象にしています。
任意のメディア操作や、ユーザーが追加するランチャー / screenshot アクションは、`playerctl`、
`wpctl`、`grim`、`wl-copy` など、該当する desktop tool が入っている環境でだけ動きます。
workspace、screenshot、app launcher などの workflow は、利用している desktop 環境に合わせて
ユーザー定義 command または script として設定してください。
