...
/**
 * デフォルト設定ファイルパスを取得
 * @returns {string} プロジェクト内の apps/desktop/sidecar/config/config.default.json
 */
export function getDefaultConfigPath(): string {
  const projectRoot = process.env.LOUPEDECK_PROJECT_ROOT || findProjectRoot(__dirname)
  return path.join(projectRoot, 'apps/desktop/sidecar/config/config.default.json')
}
