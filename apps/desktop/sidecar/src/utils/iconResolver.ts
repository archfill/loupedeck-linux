...
export class IconResolver {
  ...
  private static getDataDirs(): string[] {
    const dirs = new Set<string>()
    const xdgDataHome = process.env.XDG_DATA_HOME
    const home = process.env.HOME

    if (xdgDataHome) {
      dirs.add(xdgDataHome)
    } else if (home) {
      dirs.add(join(home, '.local/share'))
    }
    ...
    dirs.add('/run/current-system/sw/share')
    dirs.add('/usr/share')
  ...
  private static getSearchPaths(appName: string, preferredSize: number): string[] {
    const sizes = [preferredSize, 64, 48, 128, 256, 512, 32, 24, 16]
    const themes = ['hicolor', 'Papirus']
    const extensions = ['png', 'svg']
    const paths: string[] = []

    for (const dataDir of this.getDataDirs()) {
      for (const theme of themes) {
        for (const size of sizes) {
          for (const extension of extensions) {
            paths.push(
              join(dataDir, 'icons', theme, `${size}x${size}`, 'apps', `${appName}.${extension}`)
            )
            paths.push(
              join(
                dataDir,
      ...
      paths.push(join(dataDir, 'pixmaps', `${appName}.png`))
      paths.push(join(dataDir, 'pixmaps', `${appName}.svg`))
    }
