 ...
 */
export function extractPhysicalButtonConfigs(config: Config): Map<number, ButtonComponent> {
  const buttonMap = new Map<number, ButtonComponent>()
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (
      ...
      'type' in buttonConfig &&
      buttonConfig.type === 'button'
    ) {
      buttonMap.set(i, buttonConfig)
    }
  }
