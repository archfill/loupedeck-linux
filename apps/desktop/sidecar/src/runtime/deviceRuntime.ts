...
import { NotificationHandler } from '../handlers/NotificationHandler.ts'
import { AppLauncher } from '../utils/appLauncher.ts'
import { NotificationListener } from '../utils/notificationListener.ts'
import { logger } from '../utils/logger.ts'
import { AUTO_UPDATE_INTERVAL_MS } from '../config/constants.ts'
import {
  watchConfig,
  stopWatchingConfig,
  clearConfigCache,
...
import type { MediaPlayPauseButton } from '../components/MediaPlayPauseButton.ts'
...
async function buildComponents(
  ...
  physicalButtonConfigs: Map<number, ButtonComponent>
}> {
...
export function createDeviceRuntime(): DeviceRuntime {
  const loupedeckDevice = new LoupedeckDevice()
  let teardownDeviceState: () => Promise<void> = async () => {}
  ...
  async function start(): Promise<void> {
    try {
      ...
      teardownDeviceState = async (): Promise<void> => {
        // 自動更新を停止
      ...
      async function setupDeviceState(isReconnect = false): Promise<void> {
        ...
        watchConfig(undefined, async () => {
          ...
          try {
            ...
            const updatedPhysicalButtonConfigs = result.physicalButtonConfigs
  ...
  return {
    getConnectionState: () => loupedeckDevice.getConnectionState(),
    setupExitHandlers: (cleanupCallback?: () => void | Promise<void>) => {
